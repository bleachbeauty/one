...
def train(train_loader, valid_loader, num_labels, index2tgt):
    ...
    if Config.model_name in Config.model_list_nopretrain:
        ...
        if Config.continue_train:
            ...
            #model = model.from_pretrained('D:\PTM\roberta-classical-chinese-base-char', config=model_config)
            #model = model(model_config).load_state_dict(torch.load('D:\PTM\roberta-classical-chinese-base-char\pytorch_model.bin'))
...
def map_model(name):
    ...
    if name == 'lstm_crf':
        ...
        model = RoBertaCRF
    elif name == 'KoichiYasuoka/roberta-classical-chinese-large-char_crf':
        model = RoBertaCRF        
