    ...
    def read_dataset(self, mode='train'):
        ...
        if mode == 'train':
            ...
            for i in fr.readlines():
                line_t.append(i.strip())
            for i in fw_l.readlines():
                label_t.append(i.strip())
            for i in range(len(line_t)):
                res.append([line_t[i], label_t[i]])
            for (i, line) in enumerate(res):
                src.append(line[0].split(' '))
                tgt.append(line[1].split(' '))
            # 添加到数据字典
            dict_train['train'] = {'src':src, 'tgt':tgt}    
            dict_data.update(dict_train)
            ...
            for (i, line) in enumerate(res):
                ...
                tgt.append(line[1].split(' '))
            # 添加到数据字典
            dict_dev['dev'] = {'src':src, 'tgt':tgt}    
            dict_data.update(dict_dev)                 
            ...
            for (i, line) in enumerate(res):
                ...
                tgt.append(line[1].split(' '))
            # 添加到数据字典
            dict_test['test'] = {'src':src, 'tgt':tgt}    
            dict_data.update(dict_test)                    
            return dict_data
        elif mode == 'eval':
            fr = open(os.path.join(data_dir, 'dev.txt'), encoding='utf-8')
            fw_l = open(os.path.join(data_dir, 'dev-lable.txt'), encoding='utf-8')        
